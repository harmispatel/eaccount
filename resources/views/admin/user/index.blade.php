                        ...
                        <form class="actionForm" action="{{ route($ParentRouteName.'.active.action') }}"
                            ...
                            <div class="row body">
                                ...
                                <div class="margin-bottom-0 col-md-2 col-lg-2 col-sm-2">
                                    <div class="form-group">
                                        <div class="form-line">
                                            <select class="form-control" name="user_type_filter" id="user_type_filter">
                                                <option value="all">All</option>
                                                @foreach(App\RoleManage::orderBy('created_at','desc')->get() as $role)
                                                    <option value="{{ $role->id  }}" @if($userType == $role->id) selected @endif>{{ $role->name  }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
    ...
    <script>
        ...
        $(function() {
            $('#user_type_filter').change(function(){
                var val = $('#user_type_filter').val();
                var token = $('[name=_token]').val();
                $.ajax({
                    url: "{{ route($ParentRouteName.'.active.usertype') }}",
                    type: "get",
                    data : {"userType":val,"_token":token},
                    success: function (response) {
                        window.location.href="{{ url('/') }}"+"/user/active/usertype?_token="+token+"&userType="+val;
                    },
